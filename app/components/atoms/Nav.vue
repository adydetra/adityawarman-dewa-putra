...
interface Link { to: string; title: string; icon: string }
...
function isActive(to: string) {
  return route.path.replace(/\/$/, '') === to.replace(/\/$/, '');
}
</script>

<template>
  <div class="flex items-center absolute top-4 left-7 text-neutral-400 dark:text-neutral-500 text-xs gap-2 font-semibold pl-0 p-2 transition-colors ease-in-out duration-500">
