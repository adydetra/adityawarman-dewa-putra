<template>
  <p class="text-black dark:text-neutral-300 text-2xl md:text-4xl font-bold transition-colors ease-in-out duration-500">
    A.<span class="text-4xl">Dewa</span> Putra
  </p>
  <div class="mt-6 text-neutral-400 text-sm md:text-base !leading-8">
    <div class="space-y-4">
      <h1>
        ~ Hello, I am Adityawarman <span class="px-1 rounded-md bg-neutral-200/50 dark:bg-neutral-900 transition-colors ease-in-out duration-500">Dewa</span> Putra. Full Stack Developer, big fan open-source, enjoying life.
      </h1>
      <h2>
        Study at
        <AtomsLink to="www.ut.ac.id/" title="Universitas Terbuka." />
      </h2>
      <h3 class="mb-4">
        Working at <AtomsLink to="www.aptronim.com" title="Aptronim" />, As a member of the <AtomsLink to="www.aptronim.com/aptrocode" title="Aptrocode" /> team.
      </h3>
    </div>
    <p class="mt-4">
      Outside of programming, I like listening music on
      <AtomsLink to="open.spotify.com/user/31ayzku5aj7g7grortm6lca74ufq?si=f308a6ee6a2f4710" title="Spotify" />,
      looking cool or minimalist photos on
      <AtomsLink to="pinterest.com/adydetra" title="Pinterest" />,
      and <AtomsLink to="www.google.com/search?q=aquascaping" title="aquascaping" />.
    </p>
    <p class="mt-4">
      Join
      <AtomsLink to="discord.gg/9z6APrYMAq" title="Discord" />
      to play games together.
    </p>
    <div class="mt-12">
      <p>Find Me On</p>
      <AtomsSocialMedia class="gap-8 md:gap-7 mt-2" />
      <p class="mt-8">
        If you enjoy my works, consider sponsoring me on
        <AtomsLink to="github.com/sponsors/adydetra" title="GitHub Sponsor 🖤" />
      </p>
    </div>
    <AtomsFooter />
  </div>
</template>
